...
std::string AggressiveBot::Decide(GameManager* gamemanager,const std::vector<std::vector<GameObject*>>& board,int playerNum) {

    Tank* selfTank = gamemanager->GetPlayerTank(playerNum);

    Tank* enemyTank = gamemanager->GetPlayerTank(playerNum == 1 ? 2 : 1);
...
std::string DefensiveBot::Decide(GameManager* gamemanager,const std::vector<std::vector<GameObject*>>& board,int playerNum) {
    Tank* selfTank = gamemanager->GetPlayerTank(playerNum);
    if (!selfTank) {
        std::cerr << "error: null tank pointer for player " << playerNum << std::endl;
        return "f";
